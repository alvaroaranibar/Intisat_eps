...
uint8_t i2c_tx_buffer[I2C_TX_BUFFER_MAX_SIZE] = {0xFE};

uint8_t i2c_tx_buffer_data_size = 1;
    ...
    switch(__even_in_range(UCB2IV, 12))
    {
        ...
        case USCI_I2C_UCTXIFG:
            ...
            if (i2c_tx_buffer_index == i2c_tx_buffer_data_size)
            {
                ...
                /* Each time a full bit burst is transmitted, the i2c_tx_buffer is filled with 0xFF. 
                    This is the default transmission if the i2c_tx_buffer is not filled because the data is not ready. */
                    
                i2c_tx_buffer[0] = 0xFE;
                i2c_tx_buffer_data_size = 1;
            }
            break;
        case USCI_I2C_UCSTPIFG:
            i2c_rx_data_size = i2c_rx_buffer_index;
            i2c_rx_buffer_index = 0;
            /* Reset tx_buffer_index if OBC suddenly aborts communication while EPS was transmitting an array  */
            /* and ensure next attempt starts in 0. */
            i2c_tx_buffer_index = 0;    
