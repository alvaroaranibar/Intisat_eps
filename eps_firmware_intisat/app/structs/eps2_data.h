...
typedef enum
{
    ...
    EPS2_PARAM_ID_FW_VERSION                = 41,
    EPS2_PARAM_ID_MPPT_1_MODE               = 42,
    EPS2_PARAM_ID_MPPT_2_MODE               = 43,
    EPS2_PARAM_ID_MPPT_3_MODE               = 44,
    EPS2_PARAM_ID_BAT_HEATER_1_MODE         = 45,
...
/**
 * \brief EPS beacon indexes
 */
extern uint8_t eps_param_id_beacon[EPS_BEACON_NUMBER_PARAM];
 ...
 *
 * \param[in] value is the data that will overwrite the previous data.
 *
...
/**
 * \brief Function to get the size of an EPS parameter by ID.
 *
 * \param[in] id is the parameter identifier.
 * \param[out] size is the pointer where the size (in bytes) will be stored.
 *
 * \return 0 if success, -1 if the ID is unknown.
 */
